...
public class TransfersServiceTests
{
    ...
    [Fact]
    public async Task TransferDifferentCurrenciesTest()
    {
        // Arrange
        var sourceAccount = DataGenerator.RandomAccount(Guid.NewGuid(), "USD");
        var targetAccount = DataGenerator.RandomAccount(Guid.NewGuid(), "EUR");
        var transfersRepositoryMock = new Mock<ITransfersRepository>();
        var accountsRepositoryMock = new Mock<IAccountsRepository>();
        var entriesRepositoryMock = new Mock<IEntriesRepository>();
        var unitOfWorkMock = new Mock<IUnitOfWork>();
        ...
        // Act
        await Assert.ThrowsAsync<DifferentCurrencyAccountsException>(() => transfersService.TransferAsync(sourceAccount.OwnerId, sourceAccount.Id, targetAccount.Id, 100));
    ...
    [Fact]
    public async Task SameAccountTransferTest()
    {
        // Arrange
        ...
        var entriesRepositoryMock = new Mock<IEntriesRepository>();
        var unitOfWorkMock = new Mock<IUnitOfWork>();
        accountsRepositoryMock.Setup(r => r.GetWithLockAsync(account.Id)).ReturnsAsync(account);
        
        var transfersService = new TransfersService(transfersRepositoryMock.Object, entriesRepositoryMock.Object, accountsRepositoryMock.Object, unitOfWorkMock.Object);
        
        // Act
        await Assert.ThrowsAsync<SameAccountTransferException>(() => transfersService.TransferAsync(account.OwnerId, account.Id, account.Id, 100));
        
        // Assert
        accountsRepositoryMock.Verify(r => r.GetWithLockAsync(account.Id), Times.Exactly(2));
    }
}